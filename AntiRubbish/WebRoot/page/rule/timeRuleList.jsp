...
<script  type="text/javascript">
    ...
    function  stopRuleApply(id,pageNum){ 
    	str= "是否停用该记录？";
	  	if (confirm(str)) {
	  		document.allDateSearch.action = "<%=basePath%>ruleAction!ruleOperation.action?ruleID="+ id+"&opType=1&type=2";	
    ...
    function  startApplyRule(id,pageNum){ 
       	str= "是否应用该记录？";
      	if (confirm(str)) {
      		document.allDateSearch.action = "<%=basePath%>ruleAction!ruleOperation.action?ruleID="+ id+"&opType=2&type=2";		
     		document.allDateSearch.submit(); 						
      	 }
    ...
    function  deleteRule(id,pageNum){ 
    	str= "是否删除该记录？";
	  	if (confirm(str)) {
	  		document.allDateSearch.action = "<%=basePath%>ruleAction!ruleOperation.action?ruleID="+ id+"&opType=3&type=2";	
	  		document.allDateSearch.submit(); 						
	  	 }
...
<span class="posNow">时间规则 >规则列表</span>
</div>
<form name="allDateSearch" id="allDateSearch" action="ruleAction!timeRuleList.action" method="post">
          ...
          <c:choose>
			<c:when test="${!empty ruleList}">
				<c:forEach items="${ruleList}" var="row" varStatus="status">
					<tr>
						<td><div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width:120px" title="${row.name}">${row.name}</div></td>
						<td>
						...
						<td><a href="<%=basePath %>ruleAction!timeRuleDetail.action?ruleID=${row.id}&pageNum=${page.currentpage}" >修改</a></td>
						<td>
							<c:if test="${ row.apply }">
