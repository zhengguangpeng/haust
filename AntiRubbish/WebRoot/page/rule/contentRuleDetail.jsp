...
<span class="posNow"> 规则管理 >修改内容规则</span>
</div>
<div class="mainBox">
  <form action="ruleAction!contentRuleList.action">
      ...
      <div class="p10 borderAll bg_F5F6FB">
        ...
        <p style="height:20px"></p>    	
    	<label class="pr10">
        <span>发件人:<input style="width:400px" type="text"   id="sender" name="sender" value="${rule.sender}"/></span>
        <span><select name="senderFlag"  id="senderFlag"  >
	              <option value="1" <c:if test="${rule.senderFlag}">selected="selected"</c:if>>精确匹配</option>  
	              <option value="0" <c:if test="${!rule.senderFlag}">selected="selected"</c:if>>模糊匹配</option>             
        ...
        <p style="height:20px"></p>
        <label class="pr10">
        <span>标题:<input style="width:400px" type="text"   id="title" name="title" value="${rule.title} "/></span>
        <span><select name="titleFlag"  id="titleFlag"  >
	              <option value="1" <c:if test="${rule.titleFlag}">selected="selected"</c:if>>精确匹配</option>  
	              <option value="0" <c:if test="${!rule.titleFlag}">selected="selected"</c:if>>模糊匹配</option>              
