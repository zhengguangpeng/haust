...
public class CdcStartupValidator {
	...
	public CdcStartupValidator(JdbcUrl jdbcUrl, String dbUser,
			String dbPassword, String bootstrapServers) {
	...
	private void validateDatasourceConnection() {
		logger.info("begin to validate DataSource connection");
			...
			while (i > 0) {
				try {
					...
					logger.info("Successfully tested connection for {}:{} with user '{}'",
							jdbcContext.hostname(), jdbcContext.port(),mysql.username());
					return;
				} catch (SQLException e) {
					lastException = e;
					logger.info(
							"Failed testing connection for {}:{} with user '{}'",
							jdbcContext.hostname(), jdbcContext.port(),
							mysql.username());
					i--;
					try {
						...
						throw new RuntimeException(
								"MySql validation had been interrupted!", ie);
					}
				}
			}
		...
		while (i > 0) {
			try {
				...
				try {
					...
					throw new RuntimeException(
							"Kafka validation had been interrupted!", ie);
				}
			}
		}
		throw lastException;
	...
	private KafkaConsumer<String, String> getTestConsumer() {
		...
		consumerProperties.put("offset.storage.partitions","1");
		consumerProperties.put("offset.storage.replication.factor","1");
