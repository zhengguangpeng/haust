...
public class TopicPartitionOffsets {
	...
	public String toString() {
		return new ToStringBuilder(this).append("unprocessed", unprocessed)
				.append("processed", processed).toString();
	}
	...
	public void noteOffsetCommitted(long offset) {
		unprocessed = new TreeSet<>(unprocessed.stream()
				.filter(x -> x >= offset).collect(Collectors.toList()));
		processed = processed.stream().filter(x -> x >= offset)
