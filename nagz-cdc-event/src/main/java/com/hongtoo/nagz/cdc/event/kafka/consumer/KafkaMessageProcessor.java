...
/**
 * Processes a Kafka message and tracks the message offsets that have been
 * successfully processed and can be committed
 *
 */
	...
	public void process(ConsumerRecord<String, String> record) {
		...
		handler.accept(record, (result, t) -> {
			if (t != null) {
				...
				logger.debug("Adding processed record to queue {} {}",
	...
	public Map<TopicPartition, OffsetAndMetadata> offsetsToCommit() {
		...
		while (true) {
			...
			if (record == null)
				break;
			count++;
			offsetTracker.noteProcessed(new TopicPartition(record.topic(),
					record.partition()), record.offset());
		}
		logger.trace("removed {} {} processed records from queue",
				subscriberId, count);
		return offsetTracker.offsetsToCommit();
	}

	public void noteOffsetsCommitted(
